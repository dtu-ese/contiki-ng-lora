 ...
 */
void SX1272OnTimeoutIrq( void * data );
...
void SX1272Init( RadioEvents_t *events, SX1272pins_t * pins )
{
    ...
    // Initialize driver timeout timers. This might be unnecessary.
    ctimer_set(&RxTimeoutSyncWord, 100, SX1272OnTimeoutIrq, NULL );
    ctimer_stop(&RxTimeoutSyncWord);
    ctimer_set(&TxTimeoutTimer, 100, SX1272OnTimeoutIrq, NULL );
    ctimer_stop(&TxTimeoutTimer);
    ctimer_set(&RxTimeoutTimer, 100, SX1272OnTimeoutIrq, NULL );
    ...
    case MODEM_LORA:
        {
            ...
            else
            {
                ...
                // DIO0=RxDone
                SX1272Write( REG_DIOMAPPING1, ( SX1272Read( REG_DIOMAPPING1 ) & RFLR_DIOMAPPING1_DIO0_MASK & RFLR_DIOMAPPING1_DIO3_MASK ) | RFLR_DIOMAPPING1_DIO0_00 | RFLR_DIOMAPPING1_DIO3_01);
            }
...
void SX1272Prepare(uint8_t *buffer, uint8_t size){
    ...
    // SX1272SetOpMode(RFLR_OPMODE_STANDBY);
    // clock_delay_usec(1000);

        //     printf("Package data being sent: ");
    ...
    case MODEM_LORA:
        {
            ...
            if( ( SX1272Read( REG_OPMODE ) & ~RFLR_OPMODE_MASK ) == RFLR_OPMODE_SLEEP )
            {
                SX1272SetStby( );
                clock_delay_usec(1000);
            }
...
void SX1272Transmit(uint8_t len){
    SX1272SetTx( 0 );
}
    ...
    case MODEM_LORA:
        {
            ...
            // FIFO operations can not take place in Sleep mode
            if( ( SX1272Read( REG_OPMODE ) & ~RFLR_OPMODE_MASK ) == RF_OPMODE_SLEEP )
            {
                SX1272SetStby( );
                clock_delay_usec(1000);
            }
...
void SX1272SetRx( uint32_t timeout )
{
    ...
    SX1272SetOpMode( RFLR_OPMODE_STANDBY );
    //rtimer_clock_t deadline = RTIMER_NOW() + US_TO_RTIMERTICKS(400);
    ...
    case MODEM_LORA:
        {
            /*Clear everything */
            // SX1272Write(REG_LR_IRQFLAGS, 0xf);
            //                 SX1272Write( REG_LR_IRQFLAGSMASK, RFLR_IRQFLAGS_RXTIMEOUT |
            //                                       //RFLR_IRQFLAGS_RXDONE |
            //                                       RFLR_IRQFLAGS_PAYLOADCRCERROR |
            //                                       //RFLR_IRQFLAGS_VALIDHEADER |
    ...
    if( (timeout*CLOCK_SECOND)/1000 != 0 )
    {
        ctimer_set(&RxTimeoutTimer, timeout*CLOCK_SECOND/1000, SX1272OnTimeoutIrq, NULL );
    }
    ...
    if( SX1272.Settings.Modem == MODEM_FSK )
    {
        ...
        {
            //Compared to the semtech driver, we recalculate from ms to system ticks.
            ctimer_set(&RxTimeoutSyncWord, ceil( (( 8.0 * CLOCK_SECOND/1000 * ( SX1272.Settings.Fsk.PreambleLen +\
                                                        ( ( SX1272Read( REG_SYNCCONFIG ) &\
                                                                ~RF_SYNCCONFIG_SYNCSIZE_MASK ) +\
                                                                1.0 ) + 10.0 ) /\
                                                             ( double )SX1272.Settings.Fsk.Datarate ) * 1e3 ) + 4 ), SX1272OnTimeoutIrq, NULL );
        }
    }
...
void SX1272SetTx( uint32_t timeout )
{
    //ctimer_set( &TxTimeoutTimer, timeout*CLOCK_SECOND/1000, SX1272OnTimeoutIrq, NULL );
...
void SX1272Reset( void )
{
    // Set RESET pin to 1
    gpio_hal_arch_pin_set_output(SX1272.pins.Reset);
    gpio_hal_arch_set_pin(SX1272.pins.Reset);
    // Wait 1 ms
    rtimer_clock_t deadline = RTIMER_NOW() + US_TO_RTIMERTICKS(1000);
    while (RTIMER_CLOCK_LT(RTIMER_NOW(), deadline));
//    clock_delay_usec( 1000 );

    gpio_hal_arch_clear_pin(SX1272.pins.Reset);
    gpio_hal_arch_pin_set_input(SX1272.pins.Reset);
    // Wait 6 ms
    deadline = RTIMER_NOW() + US_TO_RTIMERTICKS(6000);
    while (RTIMER_CLOCK_LT(RTIMER_NOW(), deadline));
//    clock_delay_usec( 6000 );
}
...
void SX1272WriteBuffer( uint8_t addr, uint8_t *buffer, uint8_t size )
{
    ...
    spi_select(&SX1272.pins.Spi);
    spi_write_byte( &SX1272.pins.Spi, addr | 0x80 );
    for( i = 0; i < size; i++ )
    {
        spi_write_byte( &SX1272.pins.Spi, buffer[i] );
    }
    spi_deselect(&SX1272.pins.Spi);
...
void SX1272ReadBuffer( uint8_t addr, uint8_t *buffer, uint8_t size )
{
    ...
    //NSS = 0;
    //gpio_hal_arch_write_pin(SX1272.NSS, 0 );

    spi_select(&SX1272.pins.Spi);
    spi_write_byte( &SX1272.pins.Spi, addr & 0x7F );

    for( i = 0; i < size; i++ )
    {
        spi_read_byte( &SX1272.pins.Spi, &buffer[i] );
    }
    spi_deselect(&SX1272.pins.Spi);
}
    ...
    case RF_RX_RUNNING:
        if( SX1272.Settings.Modem == MODEM_FSK )
        {
            ...
            if( SX1272.Settings.Fsk.RxContinuous == true )
            {
                ...
                ctimer_reset( &RxTimeoutSyncWord );
            }
            else
            {
                SX1272.Settings.State = RF_IDLE;
...
static rtimer_clock_t last_packet_received = 0;
static rtimer_clock_t timeAtRxDone;
rtimer_clock_t SX1272_getLastPacketTimestamp(){
    /*We must deduct the time receiving the header takes.*/
    return (last_packet_received - US_TO_RTIMERTICKS(336) );
}
...
int storepacket(uint8_t irqFlags){                    int8_t snr = 0;
                    //printf("RxDonePollPacket\nStatus: %d\n", SX1272Read(REG_LR_MODEMSTAT));
                    ...
                    ctimer_stop( &RxTimeoutTimer );
                    //SX1272Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_VALIDHEADER );
                    last_packet_received = timeAtRxDone - US_TO_RTIMERTICKS((SX1272GetTimeOnAir(MODEM_LORA, SX1272.Settings.LoRaPacketHandler.Size)));
                    /*We reverse engineer the time of arrival */
                    //last_packet_received = timeAtInterrupt - SX1272GetTimeOnAir(MODEM_LORA, SX1272.Settings.LoRaPacketHandler.Size);
...
int SX1272isReceiving(){
    /*Check if a validheader-interrupt have fired */
    static uint8_t flags;
    flags = SX1272Read( REG_LR_IRQFLAGS );
    uint8_t status = SX1272Read(REG_LR_MODEMSTAT);
    //printf("Modemstat: %d, flags: %d\n", status, flags);
    if ((flags & RFLR_IRQFLAGS_RXDONE ) == RFLR_IRQFLAGS_RXDONE){
        timeAtRxDone = RTIMER_NOW();
        //printf("About to store\n");
        return storepacket(flags);/*We are done receiving */
    }
/*
    if ((flags & RFLR_IRQFLAGS_VALIDHEADER ) == RFLR_IRQFLAGS_VALIDHEADER)
    {
        last_packet_received = RTIMER_NOW();
        // Clear Irq
        SX1272Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_VALIDHEADER );
        printf("Valid header received in pollingMode\n");
        */
        /*We happy */
   /* }  */

  if (status & ( 8 | 2 | 1)){/*Checking against the 4 lsb*/
    return 1;
  }
  return 0;
}
...
int SX1272PollPacket(){
    static uint8_t irqFlags = 0;
    irqFlags = SX1272Read( REG_LR_IRQFLAGS );
    //printf("Poll Mode\n");    
/*
    if ((irqFlags & RFLR_IRQFLAGS_VALIDHEADER ) == RFLR_IRQFLAGS_VALIDHEADER)
    {
        last_packet_received = RTIMER_NOW();
        // Clear Irq
        SX1272Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_VALIDHEADER );
    ...
    //} 
        ...
        //For debugging
        //uint8_t status = SX1272Read(REG_LR_MODEMSTAT);
        //printf("Modemstat: %d, irqflags: %d\n", status, irqFlags);


            //printf("rxRunning, irqflags: %d\n", irqFlags);
...
//    SX1272Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_VALIDHEADER );
    ...
    case MODEM_FSK:
        break;
    case MODEM_LORA:
        if((flags & RFLR_IRQFLAGS_CADDETECTED ) == RFLR_IRQFLAGS_CADDETECTED )
        {
            // Clear Irq
        ...
        if ((flags & RFLR_IRQFLAGS_VALIDHEADER ) == RFLR_IRQFLAGS_VALIDHEADER)
        {
            ...
            printf("ValidHeader Through Irq\n");
            // Clear Irq
            SX1272Write( REG_LR_IRQFLAGS, RFLR_IRQFLAGS_VALIDHEADER );
        } 
        if ((flags & RFLR_IRQFLAGS_CADDONE ) == RFLR_IRQFLAGS_CADDONE)
         {
            // Clear Irq
