...
PROCESS_THREAD(pingpong, ev, data)
{
    ...
    while(1){
        ...
        if (transmit_flag){
            ...
            rand = random_rand() & 0xFFFFF;/*Random contribution, to ensure they will eventually meet*/
            target_time = RTIMER_NOW() + US_TO_RTIMERTICKS(rand) + US_TO_RTIMERTICKS(INTERVAL/4);/*We just fire*/
            Buffer[0] = magic_number;            
        }
        ...
        RTIMER_BUSYWAIT_UNTIL_ABS(NETSTACK_RADIO.receiving_packet(), startwait, US_TO_RTIMERTICKS(INTERVAL)*4);
